...
//
// Stage 4 has no approval gate: approvals are a separate ticket (#46).

import { hasGeneratedPage } from './page-preview-logic.js';

/**
 * Whether Stage 4 can be completed: the page must be generated (the preview URL
 * is the proof) AND the pre-flight check must have run (it carries a run
 * timestamp). The reason names every missing step so the UI can hint. Pure.
