...
// (#60), and everything the grid (#61) already owns - the row builder
// (`assetRow`/`assetRecord`) and the title rule (`assetTitle`) - stays in
// assets-logic.js, so one asset has one name and one saved shape.
//
// The rule this file exists for: NOTHING IS INVENTED. A field the row does not
// carry is shown as an empty state, not as a plausible-looking value. `model`
// is the sharpest case: the Firefly tool reports no model (gate #57), so the
// recorded id is only the `x-model-version` the app asked for. It is labelled
// best-effort rather than presented as tool fact - and never as the demo's
// hardcoded "Gemini 3 / Nano Banana 2".
//
 ...
 * its query string.
 *
 * This is not the grid's `thumbnailCandidates` (#61): a thumbnail is an `<img
 * src>` that walks to the placeholder when a URL fails, while the detail view
 * READS THE BYTES - `content.da.live` answers 401 to an unauthenticated `<img>`
 * - and the same blob is what Download saves. Pure.
...
/**
 * The creative direction a row remembers.
 *
 * The sheet keeps only the names, which is enough to keep a regenerated image
 * in the same look - and to record that look again - when the caller hands over
 * no creative direction of its own. Answers `null` for a row that remembers
 * none. Pure.
 * @param {{visualStyle?: string, palette?: string}} asset
 * @returns {{visualStyle?: {name: string}, colorPalette?: {name: string}}|null}
 */
export function rowDirection(asset) {
...
export function regenerationJob(asset, creativeDirection) {
  ...
  const direction = creativeDirection || rowDirection(asset);
